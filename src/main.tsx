...
function ActivityPage({ performers, sources, run }: { performers: Performer[]; sources: Source[]; run: (op: () => Promise<unknown>, msg: string) => Promise<void> }) {
  ...
  const itemAction = async (item: Item, action: "pause" | "resume" | "stop" | "cancel" | "delete") => {
    ...
    const messages = { queue: item.status === "failed" ? "Download queued again" : "Download queued", pause: "Recording paused", resume: "Recording resumed", stop: "Recording is stopping and will be saved", cancel: "Recording cancelled", delete: "Activity item deleted" };
