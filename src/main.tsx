...
import { Activity, Box, Check, ChevronLeft, ChevronRight, CircleAlert, ClipboardPaste, CloudDownload, Database, Download, ExternalLink, FolderOpen, Gauge, Globe2, HardDrive, LayoutDashboard, Link2, LoaderCircle, PackagePlus, Pencil, Plug, Plus, RefreshCw, Search, Settings, ShieldCheck, Sparkles, Terminal, Trash2, UserRound, Users, X } from "lucide-react";
...
function ActivityPage({ performers, sources, run }: { performers: Performer[]; sources: Source[]; run: (op: () => Promise<unknown>, msg: string) => Promise<void> }) {
  ...
  const [performerId, setPerformerId] = useState(""); const [sourceDomain, setSourceDomain] = useState(""); const [search, setSearch] = useState("");
  ...
  const categories = [
    ["all", "All", totalTracked], ["active", "Active", (counts.queued ?? 0) + (counts.downloading ?? 0)], ["ready", "Ready", counts.available ?? 0],
    ["downloaded", "Downloaded", counts.completed ?? 0], ["errors", "Errors", failedCount], ["other", "Other", (counts.duplicate ?? 0) + (counts.superseded ?? 0) + (counts.deleted ?? 0)],
  ] as const;
  ...
  return <section className="panel activity-panel">
    ...
    {loading && !data ? <div className="activity-loading"><LoaderCircle className="spin"/>Loading activity…</div> : data?.items.length ? <>
      <div className="activity-table"><div className="table-head"><span>Item</span><span>Source</span><span>Type</span><span>Status & progress</span><span>Download time</span><span>Updated</span><span></span></div>{data.items.map((item) => { const source = sources.find((entry) => entry.id === item.sourceId); const percent = Math.max(0, Math.min(100, Math.round(item.progress * 100))); const downloadedBytes = Number(item.downloadedBytes ?? 0); return <div className="table-row" key={item.id}><div><strong>{item.title || item.id}</strong><small>{performers.find((p) => p.id === item.performerId)?.name}</small></div><span className="activity-source"><span>{source?.label || source?.domain || "Unknown source"}</span>{source?.domain && source.label !== source.domain && <small>{source.domain}</small>}</span><span className="capitalize">{item.mediaType}</span><span className="activity-status"><span className={`badge ${item.status}`}>{item.status}</span>{item.status === "downloading" && <span className="progress-wrap"><span className={`download-progress ${percent === 0 ? "indeterminate" : ""}`} role="progressbar" aria-label={`Download progress for ${item.title || item.id}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent || undefined}><i style={percent ? { width: `${percent}%` } : undefined}/></span><small>{percent ? downloadedBytes ? `${percent}% · ${formatBytes(downloadedBytes)}` : `Preparing · ${percent}%` : downloadedBytes ? formatBytes(downloadedBytes) : "Preparing extractor…"}</small></span>}</span><span className="download-time">{downloadTime(item, clock)}</span><span>{timeAgo(item.updatedAt)}</span><span>{["available", "failed"].includes(item.status) && <button className="icon-button" title={item.status === "failed" ? "Retry download" : "Queue download"} onClick={async () => { await run(() => api(`/api/items/${item.id}/queue`, { method: "POST" }), item.status === "failed" ? "Download queued again" : "Download queued"); await loadActivity(); }}><Download size={16}/></button>}</span>{item.error && <p className="row-error">{item.error}</p>}</div>; })}</div>
